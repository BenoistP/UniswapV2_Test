...
const init = async () => {
  ...
  if (PRIVATE_KEY) {
    const msg = `PRIVATE_KEY is ${
      PRIVATE_KEY === undefined ? "UNDEFINED ❌" : " DEFINED ✔️"
    }`;
    console.log(msg);
  } else {
    console.error(msg);
  ...
  const pair = await Fetcher.fetchPairData(dai, weth, RPC_PROVIDER)
	console.debug(`pair =`);
	console.dir(pair);
